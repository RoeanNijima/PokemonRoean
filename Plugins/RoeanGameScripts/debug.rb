...
def log_special_species
  list = []
  GameData::Species.each_species do |species|
    next if species.form != 0
    next unless species.has_flag?("Legendary") || species.has_flag?("Mythical") || species.has_flag?("UltraBeast")
    next if species.has_flag?("ExcludeFromDex")
    list << [species.species, species.base_stat_total]
  end

  # Sort by catch rate descending (common first)
  list.sort_by! { |s| -s[1] }

  File.open("special_species_log.txt", "w") do |f|
    f.puts("{")
    list.each_with_index do |(name, rate), i|
      comma = (i < list.length - 1) ? "," : ""
      f.puts("  :#{name} => #{rate}#{comma}")
  ...
  pbMessage("Log saved to special_species_log.txt")
end
